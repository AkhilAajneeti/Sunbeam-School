 ...
 */
/**
 * ⚠ SEVEN GLOBS, MERGED — one per asset root, and they must stay literal.
 ...
 *
 * A folder name only has to be unique ACROSS ALL SEVEN roots, because the lookup
 * key is the folder alone.
 *
 * ⚠ THE TWO `sunbeam-assets-*` ROOTS ARE THE CLIENT'S DELIVERY FOLDERS, LEFT
 * UNDER THE NAMES THEY ARRIVED WITH. Twenty-nine galleries came in two drops —
 * `sunbeam-assets-1` (4 folders) and `sunbeam-assets-2` (25). Renaming or
 * re-filing them would mean re-doing the work at every future drop and would
 * break the client's own reference to "the folder we sent"; the resolver keys on
 * the folder name alone, so the root a gallery lives under never has to be
 * stated anywhere else in the codebase.
 *
 * ⚠ `Art Integrated Learning` EXISTS IN BOTH DROPS AND THAT IS DELIBERATE, NOT A
 * COLLISION TO FIX. The key is the folder name, so the two drops MERGE into one
 * sixteen-photograph gallery — which is the right outcome: they are the same
 * activity photographed twice. Anything that must stay separate needs a distinct
 * folder name, and the 2024–25 Ardent Reader drop is named for its session
 * precisely so it does not merge with the earlier `ardent-reader-award`.
 */
const files: Record<string, { default: ImageMetadata }> = {
  ...
  ...import.meta.glob<{ default: ImageMetadata }>(
    '../assets/sunbeam-assets-1/**/*.{jpg,jpeg,png,JPG,JPEG,PNG,webp}',
    { eager: true },
  ),
  ...import.meta.glob<{ default: ImageMetadata }>(
    '../assets/sunbeam-assets-2/**/*.{jpg,jpeg,png,JPG,JPEG,PNG,webp}',
    { eager: true },
  ),
};
...
const ROOTS = [
  ...
  '/assets/sunbeam-assets-1/',
  '/assets/sunbeam-assets-2/',
];
