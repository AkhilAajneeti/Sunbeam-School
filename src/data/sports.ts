 ...
 *
 * FOR MOST OF THIS PAGE THERE IS NO ACTION PHOTOGRAPH of a child playing a
 * sport — nearly every ground photograph is empty. That fact is why this page is
 * built the way it is: sections that would normally lean on action photography (the games
 ...
 *
 * ⚠ TWO OF THE EIGHT NOW CARRY A REAL ACTION FRAME, AND THEIR CAPTIONS CHANGED
 * WITH THEM. The client's asset drop included the District Junior Volleyball
 * championship played on the school's own court, and the junior skating squad on
 * the paved ground. Where the picture shows the sport being played, `venue` says
 * so — the caption describes THE PHOTOGRAPH, which is the rule below, and it
 * would now be wrong to caption a match as an empty field. The other six keep
 * their ground shots and their ground captions, because that is still what they
 * are. Karate, archery, handball and the hockey cluster squad also arrived in
 * that drop; none of them is one of the eight outdoor games photographed at its
 * own venue, so they are published as galleries under School Activities rather
 * than forced onto a card whose caption could not stay true.
 *
 * ═══ SOURCING ═══════════════════════════════════════════════════════════════
 *
...
import volleyballMatch from '../assets/sunbeam-assets-2/District-Junior-Volleyball/DSC_0484.JPG';
import skatingRun from '../assets/sunbeam-assets-2/Scating/DSC_4090.JPG';
...
export const games: { outdoor: Game[]; indoor: Game[] } = {
  outdoor: [
    ...
    },
    {
      ...
      venue: 'District Junior Volleyball Championship, on the school court',
      photo: volleyballMatch,
      alt: 'Teams lined up on the marked volleyball court at Sunbeam School Ballia for the District Junior Volleyball Championship',
    },
    {
      name: 'Kho-Kho',
      icon: 'khokho',
    ...
    },
    {
      ...
      venue: 'The junior squad on the paved ground',
      photo: skatingRun,
      alt: 'Junior pupils in helmets and pads skating across the paved ground at Sunbeam School Ballia',
    },
  ],
  indoor: [
    {
